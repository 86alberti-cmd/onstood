...
export function parseCreatorJson(raw){ return normalizeAcademicWork(raw,'OnStood Academic Work'); }
export function creatorQualityReport(work,pages='8-10',slides='10-12'){
  ...
  const rawLeak=JSON.stringify(normalized.sections).includes('\"sections\"') || normalized.sections.some(s=>(s.paragraphs||[]).some(p=>/^\s*[\[{].*[}\]]\s*$/.test(p)));
  ...
  return {ok:issues.length===0,issues,word_count:normalized.word_count,target_words:targetWords,section_count:normalized.sections.length,reference_count:normalized.references.length,slide_count:normalized.slides.length};
}
