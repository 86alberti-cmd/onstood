...
const ONSTOOD_SEO = {
  title: 'ONSTOOD — AI-Powered Student & Academic Platform',
  description:
    'ONSTOOD connects students, knowledge and AI in one academic platform. Learn, research, collaborate, manage documents and access intelligent tools built for student life.',
  url: 'https://onstood.com/',
  siteName: 'ONSTOOD'
};
...
function installOnstoodSeo() {
  ...
  const isSensitiveAuthUrl =
    sensitiveParams.some(key => url.searchParams.has(key)) ||
    /type=(?:recovery|signup|email)/i.test(url.search);

  document.documentElement.lang = 'en';
  document.title = ONSTOOD_SEO.title;

  upsertMeta('description', ONSTOOD_SEO.description);
  upsertMeta('application-name', ONSTOOD_SEO.siteName);
  ...
  upsertMeta('og:site_name', ONSTOOD_SEO.siteName, 'property');
  upsertMeta('og:title', ONSTOOD_SEO.title, 'property');
  upsertMeta('og:description', ONSTOOD_SEO.description, 'property');
  upsertMeta('og:url', ONSTOOD_SEO.url, 'property');
  upsertMeta('twitter:card', 'summary');
  upsertMeta('twitter:title', ONSTOOD_SEO.title);
  upsertMeta('twitter:description', ONSTOOD_SEO.description);

  upsertLink('canonical', ONSTOOD_SEO.url);

  const schemaId = 'onstood-structured-data';
  ...
  schema.textContent = JSON.stringify({
    ...
    '@graph': [
      {
        '@type': 'Organization',
        '@id': `${ONSTOOD_SEO.url}#organization`,
        name: ONSTOOD_SEO.siteName,
        url: ONSTOOD_SEO.url,
        description: ONSTOOD_SEO.description
      },
      {
        '@type': 'WebSite',
        '@id': `${ONSTOOD_SEO.url}#website`,
        url: ONSTOOD_SEO.url,
        name: ONSTOOD_SEO.siteName,
        description: ONSTOOD_SEO.description,
        publisher: {
...
// ONSTOOD_STAGE: LIBRARY-AI-CREATOR-2026-09-04
const ONSTOOD_BUILD = 'V34-SEO-BRAND-ENTITY';
...
const NAV = [
  ...
  ['docs', 'OnStood Library', FolderOpen],
...
function App({ session }) {
      ...
      ? {
          ...
          title:
            knowledgeStats.total_records > 0
              ? `${knowledgeStats.total_records.toLocaleString()} total knowledge records and growing.`
              : 'Our academic knowledge keeps growing.',
          text:
            knowledgeStats.total_records > 0
              ? `ONSTOOD currently has ${knowledgeStats.total_records.toLocaleString()} total knowledge records — indexed academic knowledge, reference-only academic discovery, and eligible student contributions — and this number keeps growing.`
              : 'ONSTOOD Knowledge keeps growing through indexed academic knowledge, reference-only academic discovery, and eligible student contributions.'
        }
      : sidebarTipBase;
  ...
  const [onlineUserIds, setOnlineUserIds] =
    useState([]);
  const [chatPresenceOpen, setChatPresenceOpen] =
    useState(false);
  ...
  useEffect(() => {
    ...
    if (!session?.user?.id || !chatPresenceOpen) {
      setOnlineUserIds([]);
      return;
    }
...
<NotificationBell
      ...
      !isMobileViewport && (
        <div
          className={`onstood-chat-dock ${chatPresenceOpen ? 'is-open' : 'is-closed'}`}
          style={{
            position: 'fixed',
            right: 18,
            bottom: 18,
            width: chatPresenceOpen ? 'min(280px, calc(100vw - 24px))' : 'auto',
            zIndex: 10040
          }}
        >
          {chatPresenceOpen ? (
            ...
            <button
              type="button"
              className="onstood-chat-launcher"
              onClick={() => setChatPresenceOpen(true)}
              title="Open chat and go online"
              aria-label="Open chat and go online"
            >
              <span className="onstood-chat-launcher-icon"><MessageCircle size={18} /></span>
              <span>Chat</span>
              <i aria-hidden="true" />
            </button>
          )}
