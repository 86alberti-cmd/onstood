...
import { Mail, Paperclip, Search, Send, Trash2, X, Pencil, Check, Sparkles } from 'lucide-react';
...
export function PostOffice({
  ...
  const [editChoiceMessage, setEditChoiceMessage] = useState(null);
  ...
  async function saveEditedMessage() {
    const message = editChoiceMessage;
    const body = editMessageText.trim();
    if (!message?.id || message.sender_id !== profile.id || !body) return;
    if (body.length > 4000) { notify('Messages can contain up to 4000 characters.'); return; }
  ...
  const filteredConversations =
    conversations
      ...
      .sort((a, b) => {
        const aOnline = onlineUserIds.includes(a.other_user_id);
        const bOnline = onlineUserIds.includes(b.other_user_id);
    ...
    >
      ...
      {(compact ||
        messageCategory ===
          'chat') && (
      <div
        className={`postoffice-layout onstood-chat2030-layout${selectedConversation || requestTarget ? ' conversation-open' : ''}`}
        style={{
          display: 'flex',
          gap: 18,
          alignItems: 'stretch',
      ...
      >
        ...
        <div
          className="card postoffice-inbox-card onstood-chat2030-inbox"
          style={{
            flex:
              '1 1 300px',
            minWidth: 280,
              ...
              {filteredConversations
                .map(item => {
                  ...
                  return (
                    ...
                    <button
                      type="button"
                      className={`onstood-conversation-row ${online ? 'is-online' : 'is-offline'} ${selectedConversationId === item.conversation_id ? 'is-selected' : ''}`}
                      key={
                        item.conversation_id
                      }
                      onClick={() => {
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              ...
                              {mine &&
                                !message.deleted_at && message.message_type === 'text' && (
                                <button
                                  type="button"
                                  onClick={() => { setEditChoiceMessage(message); setEditMessageText(message.body || ''); }}
                                  className="onstood-message-action"
                                  title="Edit message"
                                >
                                  Edit
                                </button>
      ...
      {editChoiceMessage && (
        <div className="onstood-dialog-backdrop" role="dialog" aria-modal="true" onMouseDown={event => { if (event.target === event.currentTarget) { setEditChoiceMessage(null); setEditMessageText(''); } }}>
          <div className="card onstood-edit-message-dialog">
            <div className="onstood-dialog-icon"><Pencil size={18}/></div>
            <span className="eyebrow dark">EDIT MESSAGE</span>
            <h3>Refine your message</h3>
            <p className="muted">Your conversation keeps its original timestamp and shows a discreet “edited” mark.</p>
            <textarea
              autoFocus
              ...
              onKeyDown={event => {
                if (event.key === 'Escape') { setEditChoiceMessage(null); setEditMessageText(''); }
                if (event.key === 'Enter' && (event.ctrlKey || event.metaKey)) saveEditedMessage();
              }}
              style={{width:'100%',minHeight:120,marginTop:10}}
            />
            <div className="onstood-dialog-actions">
              <button type="button" className="btn subtle" onClick={() => { setEditChoiceMessage(null); setEditMessageText(''); }}>Cancel</button>
              <button type="button" className="btn primary" onClick={saveEditedMessage} disabled={!editMessageText.trim()}><Check size={15}/> Save changes</button>
            </div>
          </div>
        </div>
      )}
